    ...
    def minimax(self, board, depth, max_min, bestv):
        ...
        for i in range(6):
            if i < len(c1):
                ...
                if len(c1) < 4:
                    value = self.minimax(board, 5-len(c1), -max_min, bestvalue)  # 走下县
                else:
                    value = self.minimax(board, depth + 1, -max_min, bestvalue)  # 走下县
                if max_min == 1:
                    if value >= bestv:
    ...
    def nodelist(self, board, width, color):
        ...
        if nodes.size > 0:
            ...
            for node in nodes:
                ...
                list.append((node[0], node[1], a-b))
            max = np.array(list)
            if color == 1:
                finallist = max[np.lexsort(-max.T)]
            else:
                finallist = max[np.lexsort(max.T)]
        return finallist
...
c = ai.evaluate(chessboard, np.array([1, 8]), 1)
